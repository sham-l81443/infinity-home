...
// ✅ FIXED: moved outside component — stable reference, no re-render on every paint
const navLinks = [
  { name: "Home", href: "/" },
  { name: "Projects", href: "#projects" },
  { name: "Services", href: "#services" },
...
export default function Navbar() {
  ...
  // ✅ FIXED: cleanup fn resets overflow if component unmounts while menu is open
  // (e.g. a route change mid-open would previously leave the page un-scrollable)
  useEffect(() => {
  ...
  return (
    <>
      ...
      {/* Mobile Full Screen Menu */}
      <div
        className={`fixed inset-0 z-40 bg-[#fafafa] flex flex-col justify-center px-8 transition-all duration-700 ease-[cubic-bezier(0.22,1,0.36,1)] ${
      ...
      >
        <div className="flex flex-col gap-10">
          {navLinks.map((link, index) => (
            <Link
              ...
              className={`text-5xl font-semibold tracking-tight text-black/60 hover:text-zinc-700 transition-all duration-500 ${
        ...
        {/* Footer details */}
        <div
          className={`absolute bottom-4 left-8 right-8 flex flex-col transition-all duration-700 ease-[cubic-bezier(0.22,1,0.36,1)] ${
            isOpen ? "opacity-100 translate-y-0 delay-500" : "opacity-0 translate-y-4 delay-0"
          }`}
        >
